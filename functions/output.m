function output(object,name,location,type)
    ...
    for n=1:size(object,2)
        ...
        mdot_sand(n) = object(n).mDot3/(object(n).h_bed * object(n).w_channel);
        ...
        k(n) = object(n).k;
        if object(n).charge
            p1in(n) =  object(n).p1(1)/10^5;
            p1out(n) =  object(n).p1(end)/10^5;
            delta_p(n) = object(n).delta_p1(end)/10^5;
        else
    ...
    eps = zeros(1,20);
    dp_bed = zeros(1,20);
    fluid_degree = zeros(1,20);
   
    if size(compressor.dp,2) > 0
        dp(1) = compressor(1).dp*10^6;
        rho(1) = compressor(1).rho_p;
        eps(1) = compressor(1).eps_por;
    ...
    %additional output
    values_add_out = zeros(3,20);

    for n=1:size(object,2)
        T3m = mean(object(n).T3);
        Abed = (object(n).L_bed + object(n).l_cavity).*object(n).w_channel;
        [m_air, u_0, u_mf] = hex.air_output(compressor.dp,compressor.rho_p,compressor.p_bed,T3m,Abed,4);
        values_add_out(1:3,n) = transpose([m_air, u_0, u_mf]);
    end

    Name_add_out = ["m_air";"u";"u_mf"];
    unit_add_out = ["kg/s";"m/s";"m/s"];
   
    Name = [Name_data;Name_comp; Name_sand; Name_Rec; Name_add_out];
    unit = [unit_data; unit_comp; unit_sand; unit_Rec; unit_add_out];
    values = [values_data; values_comp; values_sand; values_Rec; values_add_out];
